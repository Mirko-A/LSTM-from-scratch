...
class LSTM:
    ...
    def backward(self, labels: list[np.ndarray]) -> None:
        ...
        for t in reversed(range(len(self.concat_inputs))):
            ...
            dsoftmax = (
                self.outputs[t] * dL - np.dot(self.outputs[t].T, dL) * self.outputs[t]
            )
            ...
            doutput = (
                tanh(self.cell_states[t])
                * dhidden_state
                * dsigmoid(self.output_gates[t])
            )
            ...
            dcell_state = (
                dtanh(self.cell_states[t])
                * sigmoid(self.output_gates[t])
                * dhidden_state
                + cell_state
            )

            dforget = (
                dcell_state * self.cell_states[t - 1] * dsigmoid(self.forget_gates[t])
            )
            ...
            dinput = (
                dcell_state
                * tanh(self.candidate_gates[t])
                * dsigmoid(self.input_gates[t])
            )
            ...
            dcandidate = (
                dcell_state
                * sigmoid(self.input_gates[t])
                * dtanh(self.candidate_gates[t])
            )
            ...
            d_concat_inputs = (
                np.dot(self.W_f.T, dforget)
                + np.dot(self.W_i.T, dinput)
                + np.dot(self.W_c.T, dcandidate)
                + np.dot(self.W_o.T, doutput)
            )
    ...
    def test(self, inputs: str, labels: str) -> tuple[str, float]:
        ...
        for i in range(len(labels)):
            prediction = idx_to_char[
                np.random.choice(vocab_size, p=probabilities[i].reshape(-1))
            ]
    ...
    def generate(self, prompt: str, length: int = 128) -> str:
        ...
        for _ in range(length):
            ...
            prediction = idx_to_char[
                np.random.choice(vocab_size, p=probabilities[-1].reshape(-1))
            ]
